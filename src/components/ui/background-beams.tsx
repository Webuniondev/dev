...
export const BackgroundBeams = React.memo(
  ({ className, maxBeams = 24 }: { className?: string; maxBeams?: number }) => {
    const [beamCount, setBeamCount] = React.useState<number>(Math.min(maxBeams, paths.length));
    ...
    React.useEffect(() => {
      ...
      const count = prefersReduced ? 8 : isMobile ? Math.min(16, maxBeams) : Math.min(maxBeams, 24);
      setBeamCount(Math.min(count, paths.length));
      setShouldAnimate(!prefersReduced);
    }, [maxBeams]);

    const sliced = React.useMemo(() => paths.slice(0, beamCount), [beamCount]);
